    ...
    while (_running) {
      ...
      try {
        ...
        for (final req in fresh) {
          if (state != null) break;
          // Decrypt is per-request: an undecryptable blob is marked handled and
          // skipped, so one bad request can't abort the batch or (via the outer
          // catch) masquerade as a network error.
